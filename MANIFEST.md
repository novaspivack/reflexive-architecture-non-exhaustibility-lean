...
| `ResidualDynamics.lean` | **SPEC_023_RG1**: **D0**; **D1** tags; **D2** **`closureIterate`** / **`In/OutsideClosureIterateImage`**; **`residualWitness_of_kernelWitness`**, forgetful **R₂** packaging; **`standingResidualBurden_*`**; **D2** fold / **R₄** upgrade **open** |
...
| `closureIterate`, `outsideClosureIterate_iff_not_in_iterate` | **`ResidualDynamics.lean`:** **D2** internal closure **iteration** + **outside-trajectory** ↔ **¬∃** iterate (**RFO**-shaped **`Set World`** operator). |
