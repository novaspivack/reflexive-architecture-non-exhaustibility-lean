...
**Mathlib:** `v4.29.1` (see `lake-manifest.json` after `lake update`).

**Program focus (parent repo):** **Layer 1** Paper D nonexistence + **D-002 abstract** witness are proved. **EPIC_011** (**SPEC_016/017**): **`ResidualEnrichment`** + bridges + **`mixedTriple`**. **EPIC_012** (**D-001**): **`require «nems-lean»`** (**path**) + **`NemS.Prelude`** + **`NemsStructuralProgramLink`**; **`EngineReflexiveMorphism`**, **`FromNativeTraces`**. **EPIC_010** (**SPEC_014**): **`RouteCanonicality.lean`**. **EPIC_013** (**SPEC_020_PT1**): proof-theoretic **`godelProvBicBarrierHypotheses`** lives in **nems-lean** (parent epic **complete**). **EPIC_014** (**SPEC_021_SC1**, **SPEC_022_RA1**): Paper 51 **comparison** + O7 **T1–T3** (**`ProvBicU123EnrichedR4Alignment.lean`**) — **integration arc frozen** (2026-03-28); more integration Lean = **new phase** (parent **[`EPIC_014_MASTER_ORCHESTRATION.md`](../specs/INCOMPLETE/IN-PROCESS/EPIC_014_SUMMIT_INTEGRATION_LAYER/EPIC_014_MASTER_ORCHESTRATION.md)** §Summit freeze). **EPIC_015** (**SPEC_023_RG1**): **dynamical** tranche + **bounded** crown **`reflexive_development_law_*`** (**[`EPIC_015_MASTER_ORCHESTRATION.md`](../specs/INCOMPLETE/IN-PROCESS/EPIC_015_RESIDUAL_GENERATIVITY_AND_REGIME_CHANGE/EPIC_015_MASTER_ORCHESTRATION.md)**). **SPEC_024_FS1** Phase 1 **Done**; **Phase 2** paper prose **done** (dynamics section + crown integration in **`The_General_Science_of_Reflexive_Systems.tex`**; theorem map **T1–T11**). **EPIC_009** (**SPEC_013_IC1**): **`require «infinity-compression»`** (**path** `../../infinity-compression/infinity-compression-lean`) + **`ICInfinityCompressionSibling.lean`**. See parent [`QUEUE.md`](../QUEUE.md).
...
| `Residuals.lean` | `ResidualClass`, `ResidualWitness`; **`residualWitness_heq_of`** (**P4** **`HEq`**); **R₁–R₄**; **`AdmissibleResidual`**; **`admissible_residual_r4_iff_barrier_pack`** |
| `Interfaces.lean` | **U₁–U₃** interfaces; **`U123BarrierData`**; **`u123BarrierData_cast`**; **D3** **field-**`↔`** + **P4** one-way **success morphisms** (**`*_of_repr_success_imp_of_right/left`**, **`u123BarrierData_pull_of_succImp_to_right`**, **`u123BarrierData_push_of_succImp_to_left`**, **`nonempty_u123BarrierData_*`**) |
...
| `AbstractModeCover/D002ResidualWitnessTarget.lean` | **D-002 proved (abstract):** **`barrierLinkedR4ResidualWitness`**; **`barrierLinkedR4ResidualWitness_heq_of_cast_pack`** (**P4**); **`d002_barrier_linked_r4_witness_holds`** |
...
| `ResidualEnrichment/Promotion.lean` | **SPEC_016_ER1 F2** + **SPEC_023_RG1 D3/P4:** **`PayloadPromotionBridge`**, **`promote_*`**; **`standingResidualBurden_enriched_payload`**, **`standingResidualBurden_enriched_sig_and_payload`**, **`enrichedR4ResidualWitness_ext`**; bridge-irrelevant + **arch-**/**engine-eq** promotion |
...
| `Adequacy.lean` | **SPEC_012_AA1** + **SPEC_023_RG1 D3**: **`d3_adequacy_aftermath_standing_burden`**, **`d3_aftermath_standing_r4_admissible_core`**, **`d3_*_of_u123BarrierData`** / **`…_regime_arch_eq_*`** (full aftermath + **R₄ core** **`d3_aftermath_standing_r4_admissible_core_of_*`**); **`CertificateWorldConsistent`**, **`honest_aftermath_carries_admissible_r4`**; imports **`ResidualDynamics`**, **`Interfaces`** |
| `InfinityCompression.lean` | **SPEC_013_IC1 (local):** **`ICKernel`**, **`ICRemainderSketch`**, **`sketchToWitness`**; **sibling** stack → **`ICInfinityCompressionSibling.lean`** |
| `ICInfinityCompressionSibling.lean` | **EPIC_009** / **SPEC_013_IC1:** **`exists_kernelWitness_of_properExtensionViaForgetful`** (**`InfinityCompression.MetaProof`**) → **`KernelWitness`/`KernelOfMap`** (**`ResidualDynamics`**) |
| `ResidualDynamics.lean` | **SPEC_023_RG1**: **D0**–**D2**; **`d1_classical_trilemma_elim`** + **`standingResidualBurden_of_classical_trilemma`** + **`d1_classical_persistentOrProperRegimeShift`** (D1 **(i)/(ii)** **tags**) + **contentful** **`hK`** + **`standingResidualBurden_of_classical_trilemma_forgetfulKernel_*`** + **U123** **regime** **`hP`**/**`hB`** (**`d1_hP_*`**, **`d1_hB_*`**, **`…_u123RegimeHypotheses`**) (**`Interfaces`**); (**`d1_hK_refinement_forgetfulKernel_*`**, **`standingResidualBurden_refinementPayload_*`**, **`ψ/χ`** vs section **`coarse`/`fine`**); **`closureOpId`** / pathological **`pathologicalAllClosureSuccessUnit`** + **`exists_ReflexiveArchitecture_forall_closureSuccess_not_closureIterateAdequacy`** (**naive** bridge **refuted**); **`JointR2R4ResidualCertificate`** + **`nonempty_jointR2R4ResidualCertificate`**; iterate **algebra** + **iterate-backed**; **`residualWitness_of_kernelWitness`** + **`residualWitness_of_kernelWitness_ne_barrierLinkedR4ResidualWitness`** (**no** literal **R₂=R₄** merge); **`standingResidualBurden_*`**; **carrier** **`α`** stays **`Type _`** (no **`variable {α : Type}`** shadowing **D0**) |
...
| `d3_adequacy_aftermath_standing_burden_of_u123BarrierData`, `…_of_nonempty_u123_arch`, `…_of_regime_arch_eq_u123`, `…_of_regime_arch_eq_nonempty_u123` | **`Adequacy.lean`:** **D3** from **`U123BarrierData`**, **`Nonempty`**, or **`RegimeSnapshot.arch`** (**`a.arch = s.arch`**) + **cast** — dynamics **U123** hypotheses **→** aftermath **burden**. |
| `d3_aftermath_standing_r4_admissible_core_of_u123BarrierData`, `…_of_nonempty_u123_arch`, `…_of_regime_arch_eq_u123`, `…_of_regime_arch_eq_nonempty_u123` | **`Adequacy.lean`:** same entry points, **weakens** to **`∃ w, IsR4 ∧ Admissible`** (**`d3_aftermath_standing_r4_admissible_core`** chain). |
...
| `diagonalRepresentationalInterface_of_repr_success_imp_of_right`, `…_of_left`, `closureObstructionInterface_*`, `semanticCertificationInterface_*`, `u123BarrierData_pull_of_succImp_to_right`, `u123BarrierData_push_of_succImp_to_left`, `nonempty_u123BarrierData_pull_of_succImp_to_right`, `nonempty_u123BarrierData_push_of_succImp_to_left` | **`Interfaces.lean`:** **P4** one-way success morphisms (**non-iff** D3 transport). |
| `residualWitness_heq_of` | **`Residuals.lean`:** **`HEq`** on **`ResidualWitness`** from class + carrier + witness **`HEq`**. |
| `barrierLinkedR4ResidualWitness_heq_of_cast_pack` | **`D002ResidualWitnessTarget.lean`:** **`HEq`** of barrier-linked witnesses from **`A = A'`** + **`u123BarrierData_cast`**. |
| `standingResidualBurden_enriched_payload`, `standingResidualBurden_enriched_sig_and_payload`, `enrichedR4ResidualWitness_ext` | **`Promotion.lean`:** **σ**/**payload** **`StandingResidualBurden`** + same-**`A`**,**`F`** extensionality. |
| `exists_kernelWitness_of_properExtensionViaForgetful` | **`ICInfinityCompressionSibling.lean`:** **IC** **DQ4.4** fiber collision ⇒ **`KernelWitness`**. |
...
| `closureIterateAdequacy_iterateBacked`, `closure_obstruction_of_iterateBacked_and_universal_outside` | **`ResidualDynamics.lean`:** **iterate-backed** architecture ⇒ definitional **`ClosureIterateAdequacy`**; packaged obstruction. |
| `exists_admissible_r2_and_barrier_linked_r4_joint` | **`ResidualDynamics.lean`:** simultaneous **IC** **R₂** + **D-002** barrier **R₄** (coexistence). |
| `JointR2R4ResidualCertificate`, `jointR2R4ResidualCertificate`, `nonempty_jointR2R4ResidualCertificate` | **`ResidualDynamics.lean`:** one **certificate** packaging canonical **R₂** + **R₄** equalities + **`r*_props`** lemmas; **`Nonempty`** via canonical **`jointR2R4ResidualCertificate`**. |
| `closureIterateAdequacy_of_global_closure_failure`, `pathologicalAllClosureSuccessUnit`, `not_closureIterateAdequacy_pathologicalUnit`, `not_closureIterateAdequacy_pathologicalUnit_empty_univ`, `exists_ReflexiveArchitecture_forall_closureSuccess_not_closureIterateAdequacy` | **`ResidualDynamics.lean`:** **vacuous** vs **sharp** countermodel to **free** **`ClosureIterateAdequacy`**; **existential** packaging (**`∀ Cl, closure_success`** \(\nRightarrow\) iterate **adequacy** for fixed **`S₀`/`B`**). |
| `closureOpId`, `closureIterate_closureOpId`, `not_closureIterateSoundness_of_iterate_fixed_seed_ne_target` | **`ResidualDynamics.lean`:** identity iterate / **no-go** for soundness when seed stays put but **≠** target. |
| `d1_classical_trilemma_elim`, `d1_response_step_classical_trilemma`, `d1_classical_persistentOrProperRegimeShift`, `standingResidualBurden_of_classical_trilemma` | **`ResidualDynamics.lean`:** **strong D1** case-split scaffold from **LEM** trilemma; **binary** **(i)/(ii)** **tags** (**persistent-or-proper**); **`StandingResidualBurden (K ∨ P ∨ B)`** via explicit **`@`** application (**implicit** **`step`** precedes **`h₁..h₃`** in binding order). |
| `reflexive_development_law_standing_trilemma`, `reflexive_development_law_fold_obstruction_iterate_geometry`, `reflexive_development_law_fold_obstruction_iterateBacked`, `reflexive_development_law_fold_obstruction_opaqueAttempt` | **`ResidualDynamics.lean`:** **SPEC_024_FS1** / **SPEC_023_RG1** §Crown — **`sorry`-free** **bounded** packaging (**D1** standing trilemma + **D2** fold / **`ClosureObstructionInterface`**); same proofs as **`standingResidualBurden_of_classical_trilemma`** / **`closure_obstruction_of_*`**; **no** added universal quantifiers. |
| `standingResidualBurden_refinementPayload_forgetfulKernel_r2_bundle`, `d1_hK_refinement_forgetfulKernel_r2_bundle_of_refineStillFine` | **`ResidualDynamics.lean`:** **D1** refinement branch + **IC** **R₂** bundle (**`α β : Type`**); **`hK`** conclusion is **`∃ ψ χ hstep, step = refinement … ∧ χ x y ∧ ∃ rw, …`** (**no** section **`fine`** capture). |
| `standingResidualBurden_refinementPayload_forgetfulKernel_joint_r2_r4`, `d1_hK_refinement_forgetfulKernel_joint_r2_r4_of_refineStillFine` | **`ResidualDynamics.lean`:** same + **`U123BarrierData`** **R₄** coexistence in the existential tail. |
| `standingResidualBurden_of_classical_trilemma_forgetfulKernel_r2`, `…_joint_r2_r4`, `…_r2_raw`, `…_joint_r2_r4_raw` | **`ResidualDynamics.lean`:** **LEM** trilemma + forgetful **`hK`**; **`hP`**/**`hB`** parameters or **`id`** (**raw** proper / bookkeeping disjuncts); **`@standingResidualBurden_of_classical_trilemma`** for implicit order. |
| `nonempty_u123BarrierData_regime_after_of_arch_eq`, `d1_hB_reconfiguration_bookkeeping_u123_nonemptyTransport`, `d1_hP_proper_regimeChange_pair_u123` | **`ResidualDynamics.lean`:** **`u123BarrierData_cast`** on **bookkeeping** **`RegimeSnapshot`**; hypothesis-shaped **`hB`**/**`hP`** strengthening (**SPEC_003_BT1** / **D3** hook). |
| `standingResidualBurden_of_classical_trilemma_forgetfulKernel_joint_r2_r4_u123RegimeHypotheses`, `…_r2_u123RegimeHypotheses` | **`ResidualDynamics.lean`:** one-shot **LEM** disjunction: forgetful **`K`** + **paired** / **transported** **`Nonempty (U123BarrierData …)`** on **reconfiguration** branches. |
| `standingResidualBurden_enriched_obstruction_sig` | **`Promotion.lean`:** **σ** = **`ObstructionSignature`** slot ↔ **`StandingResidualBurden`** (**strong D1** hook). |
...
| `residualWitness_of_kernelWitness`, `admissibleResidual_residualWitness_of_kernelWitness`, `residualWitness_of_kernelWitness_ne_barrierLinkedR4ResidualWitness` | **`ResidualDynamics.lean`:** **R₂** **`ResidualWitness`** from **`KernelWitness`** (admissible; **≠** **`trivialR4`**); **literal** **Eq** with **`barrierLinkedR4ResidualWitness`** **refuted**; honest **coexistence** = **`JointR2R4ResidualCertificate`** / joint existentials. |
...
| `closureIterate`, `closureIterate_add`, `inClosureIterateImage_trans`, `outsideClosureIterate_iff_not_in_iterate` | **`ResidualDynamics.lean`:** **D2** internal closure **iteration**, **additivity** of step count, **transitivity** of **`InClosureIterateImage`**, **outside-trajectory** ↔ **¬∃** iterate (**RFO**-shaped **`Set World`** operator). |
