...
| `ResidualDynamics.lean` | **SPEC_023_RG1**: **D0**–**D2**; **`d1_classical_trilemma_elim`**; **`closureOpId`** / pathological **`pathologicalAllClosureSuccessUnit`**; **`JointR2R4ResidualCertificate`**; iterate **algebra** + **iterate-backed**; **`residualWitness_of_kernelWitness`**; **`standingResidualBurden_*`**; **KernelWitness→R₄** **one witness** **open** |
...
| `JointR2R4ResidualCertificate`, `jointR2R4ResidualCertificate` | **`ResidualDynamics.lean`:** one **certificate** packaging canonical **R₂** + **R₄** equalities + **`r*_props`** lemmas. |
...
| `d1_classical_trilemma_elim` | **`ResidualDynamics.lean`:** **strong D1** case-split scaffold from **LEM** trilemma. |
