...
| `ResidualDynamics.lean` | **SPEC_023_RG1**: **D0**–**D2**; **`d1_classical_trilemma_elim`** + **`standingResidualBurden_of_classical_trilemma`** + **contentful** **`hK`** + **`standingResidualBurden_of_classical_trilemma_forgetfulKernel_*`** + **U123** **regime** **`hP`**/**`hB`** (**`d1_hP_*`**, **`d1_hB_*`**, **`…_u123RegimeHypotheses`**) (**`Interfaces`**); (**`d1_hK_refinement_forgetfulKernel_*`**, **`standingResidualBurden_refinementPayload_*`**, **`ψ/χ`** vs section **`coarse`/`fine`**); **`closureOpId`** / pathological **`pathologicalAllClosureSuccessUnit`**; **`JointR2R4ResidualCertificate`** + **`nonempty_jointR2R4ResidualCertificate`**; iterate **algebra** + **iterate-backed**; **`residualWitness_of_kernelWitness`**; **`standingResidualBurden_*`**; **carrier** **`α`** stays **`Type _`** (no **`variable {α : Type}`** shadowing **D0**); **KernelWitness→R₄** **one witness** **open** |
...
| `closureIterateAdequacy_of_global_closure_failure`, `pathologicalAllClosureSuccessUnit`, `not_closureIterateAdequacy_pathologicalUnit` | **`ResidualDynamics.lean`:** **vacuous** vs **sharp** countermodel to **free** **`ClosureIterateAdequacy`**. |
...
| `d1_classical_trilemma_elim`, `standingResidualBurden_of_classical_trilemma` | **`ResidualDynamics.lean`:** **strong D1** case-split scaffold from **LEM** trilemma; **`StandingResidualBurden (K ∨ P ∨ B)`** via explicit **`@`** application (**implicit** **`step`** precedes **`h₁..h₃`** in binding order). |
