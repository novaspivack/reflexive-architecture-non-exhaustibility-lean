...
| `ResidualDynamics.lean` | **SPEC_023_RG1**: **D0**–**D2**; **`d1_classical_trilemma_elim`** + **`standingResidualBurden_of_classical_trilemma`**; **`closureOpId`** / pathological **`pathologicalAllClosureSuccessUnit`**; **`JointR2R4ResidualCertificate`** + **`nonempty_jointR2R4ResidualCertificate`**; iterate **algebra** + **iterate-backed**; **`residualWitness_of_kernelWitness`**; **`standingResidualBurden_*`**; **carrier** **`α`** stays **`Type _`** (no **`variable {α : Type}`** shadowing **D0**); **KernelWitness→R₄** **one witness** **open** |
