...
| `ResidualDynamics.lean` | **SPEC_023_RG1**: **D0**; **D1** tags; **D2** **`closureIterate`**, **`IterateBackedReflexiveArchitecture`**, **`OpaqueAttemptClosureIterateBacking`**, obstruction packaging, trajectory **algebra**; **joint** **`exists_admissible_r2_and_barrier_linked_r4_joint`** (not **R₂→R₄**); **`residualWitness_of_kernelWitness`**, forgetful **R₂**; **`standingResidualBurden_*`**; intrinsic iterate **adequacy** only via **bundles**; typed **KernelWitness→R₄** identification **open** |
