...
| `ResidualDynamics.lean` | **EPIC_015 / SPEC_023_RG1** + **SPEC_013_IC1** pattern: **`KernelOfMap`**, **`d0_forgetfulKernel_*`**, **`KernelWitness`**, **`d0_*`**; **D1** tag-level: **`d1_response_step_exhaustive`**, **`d1_not_refinement_then_reconfiguration`**, **`isRefinement_eq_true_iff`**, **`isReconfiguration_eq_true_iff`**; **strong D1** / **D2** / **D3** **open** |
