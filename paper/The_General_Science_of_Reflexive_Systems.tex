...
This is not a technology-stack paper, a repository tour, or a report on a formalization project. The formal artifacts matter because they sharpen definitions, block smuggling, and force the theory to become explicit. But the paper's subject is broader. It proposes a general science of reflexive systems, and uses formal realization to establish its claims with unusual precision. Readers outside the formal stack should treat the machine-checked layer as an audit trail for boundaries and constructions, not as the \emph{source} of the scientific problems---those problems arise wherever reflexive completion is attempted in earnest.
...
The formal stack isolates a positive residual profile that appears precisely after the failure of the canonical completion modes. Scientifically, this means that the reflexive system is not internally completed, but its noncompletion is itself organized.
...
\begin{remark}
The residual branch is not a fourth completion mode. The first three families are completion ambitions. The residual branch is an aftermath or survivor family appearing once those ambitions fail.
\end{remark}

If this distinction is blurred, the science collapses into confusion. The residual is not an alternate success doctrine. It is the lawful structure that remains after strong completion fails.
...
\paragraph{Why the ``failure is not a defect'' layer comes next.}
The aftermath theorems are easier to misread than to prove: a hurried reader may still hear only ``completion fails.'' Before turning to witness engineering and semantic depth, the paper must separate \emph{constitutive} failure from pathology and state the philosophical thesis that makes the science worthwhile: nonexhaustibility is the price and condition of nontrivial reflexive depth, not an embarrassment to hide.

\section{Why nonexhaustibility is not a defect}

The flagship impossibility theorem can be misread as a gloomy conclusion: ``reflexive systems cannot win.'' The correct scientific reading is almost the opposite for a theory of depth.
...
\subsection{Residual structure as ongoing semantic and architectural life}

Residual profiles and enriched witnesses are not consolation prizes. They are the mathematical forms of \emph{continued} reflexive structure: types of obstruction that survive, can be witnessed, and can be enriched. Semantic summit stratification and O7-style integration then show how different semantic regimes participate in the same post-completion architecture. Nonexhaustibility thus names not a wound but the \emph{space in which} a reflexive system can keep doing honest reflexive work.

\paragraph{Why witnesses and enrichment are the natural next step.}
Once aftermath is recognized as organized rather than chaotic, the science must say \emph{how} that organization is carried: what is witnessed, with what payloads, and along which obstruction columns. Without this layer, ``residual structure'' would remain a slogan; with it, the aftermath becomes operational science---the bridge from forced profiles to architectures that can respect barrier discipline in practice.

\section{Residual witnesses and enrichment}

\paragraph{Scientific inevitability.}
A science that stopped at ``a positive profile is forced'' would still lack operational content. The next question is unavoidable: \emph{how} is that profile materialized as witness data, and how does obstruction survive in typed columns? Witness theory answers that question. It is not decorative enrichment; it is the step that turns aftermath from a blanket existence claim into something an architecture can implement, audit, or falsify.
...
\subsection{Scientific meaning}

The science of reflexive systems therefore does not stop at:
\[
\text{completion fails}.
\]
It proceeds to:
\[
\text{completion fails in typed ways, and those ways survive as enriched residue.}
\]
...
\paragraph{Why semantic summit regimes come next.}
Residual theory describes how obstruction survives and is packaged \emph{after} completion pressure fails. But reflexive systems also inhabit different \emph{semantic organizations}---different senses in which self-semantic closure can be strong, weak, class-restricted, or proof-theoretically full. Without stratifying those regimes, one cannot say scientifically \emph{how strong} barrier-style closure can be in which setting. The next two sections are therefore not a detour from residuality; they complete the vertical axis of the same field.

\section{Semantic barrier regimes I: the computational predicated summit}

\paragraph{Scientific inevitability.}
Witnesses describe \emph{how} residue is carried at the obstruction layer. They do not yet specify \emph{how strong} self-semantic barrier closure can be in different organizational regimes. Without summit stratification, one cannot distinguish honest class-restricted strength from delusions of unrestricted universality---a distinction that matters as soon as reflexive systems are compared across computational and proof-theoretic implementations.
...
\subsection{Why a semantic summit is needed}

The theory so far identifies barriered completion failure and structured aftermath. But the semantic side of the story has its own summit structure. Different semantic regimes support different kinds of closure.
...
\paragraph{Why integration is not yet the culmination of the arc.}
Without the next section, a skeptical reader could still imagine three disjoint stories: a \emph{mode-cover} impossibility theater, a \emph{semantic summit} specialty track, and a \emph{residual} witness engineering track. That tripartite split would betray the scientific ambition: the same reflexive systems simultaneously press completion, hit barriers, live in semantic regimes, and leave organized residue. The point of alignment constructions is to show these are \textbf{one system}---not three bibliographies forced between the same covers. But even after integration, the science still owes an answer to the question: \emph{what happens next?} That is the dynamical layer developed after the integration arc.

\section{Integration into the residual theory}

\subsection{Why integration is necessary}

Semantic barrier summits would remain scientifically \emph{isolated} if they could not be fed back into the theory of what survives completion failure. Conversely, residual theory would be \emph{semantically blind} if it could not articulate how different barrier strengths (predicated vs full, constant vs pack-dependent seams) participate in the same enriched aftermath pipeline. \textbf{O7-style integration} is therefore not ``another bridge lemma'' in isolation; it is what keeps impossibility, semantics, and residue from fracturing into unrelated specialties.

\paragraph{What would be unfinished without it.}
Without this alignment arc, a reader could still treat the paper as three parallel specialties: \textbf{(a)} mode-cover impossibility, \textbf{(b)} semantic summit comparison, \textbf{(c)} residual witness engineering. That partition would misdescribe the subject. Real reflexive systems do not get to choose only one storyline; they face completion pressure, semantic closure strength, and residual packaging together. Integration is what makes the narrative \emph{one science} rather than a convenient anthology.
...
We can now state the field-level law developed in the paper.
...
In one arc: completion modes \(\to\) anchored impossibility \(\to\) forced aftermath \(\to\) witnesses and enrichment \(\to\) semantic summit stratification \(\to\) integration \(\to\) lawful dynamical response. That package is what earns the title \emph{general science} rather than a local impossibility note. The \emph{static} half (completion through integration) says what reflexive systems cannot do and what structure is forced; the \emph{dynamical} half (the Reflexive Development Law) says what happens next under honest barrier discipline. Together they constitute a theory of reflexive systems that \emph{persist and develop}, not merely systems that fail once.

\subsection{Relation to earlier traditions}

The historical arc and the contrast to classical limitative results are developed in the Introduction (\S1.3). The synthetic claim here is blunt: Gödel, Tarski, Kleene, and Lawvere (among others) established that diagonal and reflexive structure carries unavoidable cost. This paper's contribution is not to discard those costs but to show how they organize a \emph{field}: completion modes, joint barriers, lawful residue, semantic summit ordering, explicit integration back into post-completion architecture, and lawful dynamical response. The advance is ecological---a theory of how reflexive systems live \emph{under} limitation and continue to develop---not a single sharper paradox.

\subsection{Beyond this formal realization}

The current formal realization is rigorous, but the science is broader than the realization. Nothing in the field law requires that reflexive systems be studied only in the present abstract architecture, only in one theorem prover, or only in one programmatic stack. The present work should be read as a mathematically explicit realization of a broader theory.

\paragraph{Beyond these interface names.}
What matters scientifically is not the contingent labels of any one formal interface package, but the \textbf{recurring pattern} they make explicit: reflexive systems press toward honest internal completion, are blocked by lawful barrier families rather than mere noise, \textbf{persist through structured residue} whose semantic strength is regime-dependent, and \textbf{respond to that residue under lawful dynamical constraints}. Any future formalism that studies reflexive systems at comparable depth---using different primitives, proof technology, or application domain---will still have to confront analogues of completion pressure, barriered failure, structured residue, summit stratification, semantic-to-residual alignment, and dynamical response to standing burden, or else explain why its subject falls outside this scientific niche.

\section{Dynamics: the Reflexive Development Law}

The science so far is \emph{static}: it classifies completion modes, proves anchored impossibility, organizes aftermath, and stratifies semantic regimes. But a general science of reflexive systems must also say what happens when a system \emph{continues} under barrier discipline after forced residual formation. This section develops that \emph{dynamical} layer.

\subsection{Why a dynamical layer is scientifically required}

A reflexive system that has experienced barriered completion failure has not ceased to operate. It persists with standing residual burden. The honest scientific question is: what \emph{lawful} outcomes are available when the system responds?

Without a dynamical account, the science leaves open the possibility that standing residual could be absorbed, erased, or circumvented by some unanalyzed ``next step.'' The point of the dynamical layer is to prove that this cannot happen freely---that response to residual is itself constrained by lawful structural alternatives.

\subsection{Standing residual burden (D0: persistence)}

The first dynamical principle is that residual is not transient noise but a \emph{standing} unresolved burden.
...
The formal realization introduces this as a lightweight alias (\leanref{StandingResidualBurden}) that explicitly marks when a \texttt{Prop} carries dynamical weight.

\paragraph{Persistence under refinement.}
Kernel witnesses---fiber collisions from forgetful maps, infinity-compression geometry, or paired obstruction data---persist as nontrivial standing burden so long as the relevant identification is not fully separated by a refinement step. If a refinement does not split all kernel pairs, a nontrivial residual persists. The science does not claim that \emph{every} residual survives every refinement; it claims that \emph{non-resolving} refinement leaves standing burden intact.

\subsection{The response trilemma (D1: Reflexive Development Law)}
...
\end{enumerate}
These exhaust the structural alternatives. The resulting standing residual burden is a disjunction \(K \lor P \lor B\) where \(K\), \(P\), \(B\) are supplied by the caller's morphisms from the three branches.
\end{theorem}

\noindent The formal realization is \leanref{reflexive\_development\_law\_standing\_trilemma}. The key scientific point: there is no fourth option in which standing residual simply disappears without trace. The system either refines (preserving residual in a new observational regime), shifts to a genuinely new regulatory architecture, or performs a bookkeeping update.

\paragraph{Content of the refinement branch.}
In the refinement case, when standing residual arises from a forgetful kernel (infinity-compression geometry), the refinement branch carries an admissible \(R_2\) residual witness and, when barrier data is available, a joint \(R_2\)/\(R_4\) coexistence certificate. This is not a vacuous disjunct: it packages typed residual data through the refinement.

\subsection{Fold obstruction and regime transition (D2)}
...
\noindent The formal realization is \leanref{reflexive\_development\_law\_fold\_obstruction\_iterate\_geometry} (and its variants for iterate-backed architectures and opaque-attempt backings).

\paragraph{Scientific meaning.}
When a reflexive system's closure regime is iterate-backed and the iterate geometry cannot reach the target, the system is \emph{forced} into a closure obstruction---it cannot pretend the current regime still works. This is the formal content of \emph{regime transition under fold obstruction}: the obstruction is not a gap in the theory but a proved structural consequence of the iterate geometry.

\paragraph{Sharp boundary.}
The paper proves that iterate-backed adequacy is a genuine \emph{commitment}, not a free property of all reflexive architectures: there exist architectures where every closure operator is declared successful but no iterate-backed adequacy holds for any seed/target pair (\leanref{exists\_Reflexive\-Architecture\_forall\_closureSuccess\_not\_closureIterate\-Adequacy}). This sharp boundary prevents the fold obstruction theorem from being vacuously applied.

\subsection{Adequacy burden relocation (D3)}

The third dynamical principle ties the dynamical layer back to the adequacy science.

Under the triple barriers, the aftermath theorem (\S4) forces admissible \(R_4\) witnesses. The dynamical D3 bridge says that this forced burden \emph{relocates} under architecture cast and regime alignment, rather than vanishing:
...
\noindent The formal realizations are \leanref{d3\_adequacy\_aftermath\_standing\_burden} and its entry-point family (\leanref{d3\_adequacy\_aftermath\_standing\_burden\_of\_u123BarrierData}, regime-aligned variants). Weakened \(R_4\)-core variants (\leanref{d3\_aftermath\_standing\_r4\_admissible\_core}) confirm that burden persists even when only the existential ``there is an admissible \(R_4\) witness'' is retained.

\subsection{Anti-smuggling: what the dynamical layer does not claim}

The dynamical layer is bounded by the following explicit non-claims:
\begin{itemize}[leftmargin=1.5em]
...
\item It does \textbf{not} supply a universal quantifier ``for all reflexive architectures, the development law holds without hypotheses.'' The trilemma and fold theorems require caller-supplied hypotheses (response step data, iterate-backed commitment).
\item It does \textbf{not} claim that the D1 refinement branch always produces a strictly finer regime---only that the structural \emph{form} of the response is one of the three lawful alternatives.
\end{itemize}

\subsection{The static--dynamical arc as one science}

The dynamical layer is not a separate research program appended to the static theory. It is the natural completion of the same inquiry:
\[
\text{completion attempt} \;\to\; \text{barrier} \;\to\; \text{lawful residual} \;\to\; \text{lawful response}
\]
where the response itself is constrained (trilemma), its closure regime is auditable (fold obstruction), and the adequacy burden it carries is accountable (D3 relocation). The ``General Science'' claim of this paper rests on this full arc: static impossibility and aftermath are necessary but not sufficient; the dynamical layer is what makes the science \emph{complete} as a theory of reflexive systems that persist and respond, not merely systems that fail once.
...
No. The Reflexive Development Law (Theorem~\ref{theorem:rdl-trilemma}) is a proved structural trilemma, not a narrative suggestion. The fold obstruction theorem (Theorem~\ref{theorem:fold-obstruction}) is a machine-checked consequence of iterate geometry. The D3 adequacy bridge is a family of proved transport lemmas. The dynamical layer claims \emph{exactly} what its hypotheses support: that response to standing residual takes one of three lawful forms, and that iterate-backed closure regimes face explicit obstruction. It does not claim that every residual is generative or that self-improvement is automatic.
...
\noindent The Master law (opening of \S12) is the thesis in quotable form. Under the three barriers, anchored completion is impossible, yet organized residue, semantic stratification, O7 integration, and the Reflexive Development Law complete one scientific arc. The static half says what reflexive systems cannot do and what structure is forced; the dynamical half says what happens next---refinement, regime shift, or fold-obstruction transition---under honest barrier discipline. \emph{Nontrivial reflexive organization coexists with lawful nonexhaustibility}---not as defeat, but as the texture in which reflexive science proceeds and develops.
...
\item (Reflexive Development Law) Response to standing residual takes one of three lawful forms: refinement, proper regime shift, or bookkeeping reconfiguration.
\item (Fold obstruction) Iterate-backed closure adequacy plus universal outside-iterate forces closure obstruction; naive iterate adequacy is not free.
\item (Adequacy burden relocation) Forced admissible \(R_4\) aftermath constitutes standing residual burden; this burden transports across architecture casts and regime alignment.
\end{enumerate}

\section{Formal artifact note}

The formal realization for the present paper is machine-checked in \Lean{} with \Mathlib{}, across the reflexive-architecture-nonexhaustibility stack and the supporting \texttt{nems-lean} stack. The machine-checked artifact does not replace the scientific argument of this paper; it serves to make the theorem boundaries, witness constructions, integration seams, and dynamical layer explicit and auditable. The dynamical layer (\leanref{ResidualDynamics.lean}) includes the crown theorems \leanref{reflexive\_development\_law\_standing\_trilemma} and \leanref{reflexive\_development\_law\_fold\_obstruction\_iterate\_geometry} with 0~\texttt{sorry} and 0~\texttt{axiom} in the project-owned modules.
