...
\subsection{Forward reference: semantic type obstruction extends the fold-obstruction layer}
...
The core finding, proved in \texttt{reflective-fold-obstruction-lean/SemanticType/} with zero \texttt{sorry}: \textbf{Turing-completeness does not imply semantic-type completeness}. A system may be able to compute every partial recursive function while permanently type-bounded at the level of which architectural kinds it can internally reach. The formal \textbf{simulation/realization split} is machine-checked: forward simulation (a typed primitive simulation map) always projects type reachability, but the converse requires a section with backward step-lifting (\texttt{TypedPrimitiveSimulationSection}). Without that section, a Turing-complete host can encode a richer-type system without itself realizing that type natively.
